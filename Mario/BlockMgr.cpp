...
bool CBlockMgr::Collision_Block(CObj* _obj, float* _pfY)
{
	bool Chk_BlockList = true;
	
	for (size_t i = 0; i < BlockEnd; ++i)
	{
		Chk_BlockList &= m_BlockList[i].empty(); // 하나라도 값이 있으면 false
	}
	if (Chk_BlockList) // 값이 있으면 반환 X
		return false;

	CBlock*		pTargetBlock = nullptr;

	
	pTargetBlock = CCollilsionMgr::Collision_Block(_obj, m_BlockList);

	if (nullptr == pTargetBlock)
		return false;

	*_pfY = (float)pTargetBlock->Get_Rect().top;
